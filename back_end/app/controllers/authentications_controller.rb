class AuthenticationsController < ApplicationController
  ...
  def register
    ...
    if user.save
      render json: user, satus: :ok
    else
      render json: { errors: user.errors.full_messages }, satus: :unprocessable_entity
    end
  end
# We need to return this JWT token after a user Register or Logs in.
# we can generate a JWT token and send that in our login action:
  def login
